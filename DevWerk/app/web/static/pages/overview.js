...
import { emptyState, eventRow, metricCard, noProjectState, taskMiniCard, workflowPipeline } from "../ui/components.js";

export function renderOverview(state) {
  if (!state.project) return noProjectState();
  const tasks = state.board?.tasks || [];
  const running = tasks.filter((task) => ["running", "recovering", "waiting"].includes(task.status)).length;
  ...
  return `<div class="page-stack overview-page">
    ...
    <section class="metric-grid">${metricCard("Formal Tasks", String(tasks.length), "当前 Workflow 的全部任务", "blue")}${metricCard("In progress", String(running), "running / waiting / recovering", "violet")}${metricCard("Completed", String(done), "明确进入 done 终态", "green")}${metricCard("Failed", String(failed), failed ? "需要 Conversation Agent 介入" : "当前没有失败任务", failed ? "red" : "slate")}</section>
