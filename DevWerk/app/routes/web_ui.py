...
function renderAnalyticsSection() {
  ...
  $("page").innerHTML = `<div class="section-stack">
    ...
    <div class="quick-grid">${kpi("Requests", usage.request_count || 0)}${kpi("Input Tokens", compact(usage.input))}${kpi("Output Tokens", compact(usage.output))}${kpi("Total Tokens", compact(usage.total))}${kpi("Duration", duration(usage.duration))}${kpi("Tasks", allTasks().length)}</div>
...
function usageCard(u){ return `<div class="card card-pad"><div style="display:flex;justify-content:space-between"><div class="h3">Token Usage</div><span class="muted" style="font-size:12px">Backend usage DB</span></div><div style="font-size:20px;font-weight:850;margin-top:12px">${compact(u.total)} total tokens</div><div class="progress"><span style="width:${u.total ? 100 : 0}%"></span></div><div class="metric-lines"><div class="metric-line"><span>Input Tokens</span><b>${compact(u.input)}</b></div><div class="metric-line"><span>Output Tokens</span><b>${compact(u.output)}</b></div><div class="metric-line"><span>LLM Calls</span><b>${compact(u.calls)}</b></div></div>${usageBars()}</div>`; }
...
function usageBars(){ const rows=state.usage.projects || []; if(!rows.length) return `<div class="muted" style="margin-top:12px">No usage rows returned by backend.</div>`; const values=rows.slice(0,24).map(row=>Number(row.total_tokens || 0)); const max=Math.max(...values, 1); return `<div class="sparkbars">${values.map(value=>`<i style="height:${Math.max(3, Math.round((value / max) * 52))}px"></i>`).join("")}</div>`; }
...
function usageTotals(){ const rows = state.usage.projects || []; return rows.reduce((a,r)=>{ a.calls += r.calls || 0; a.input += r.input_tokens || 0; a.output += r.output_tokens || 0; a.total += r.total_tokens || 0; a.duration += r.duration_ms || 0; return a; }, {request_count: state.usage.request_count || 0,calls:0,input:0,output:0,total:0,duration:0}); }
