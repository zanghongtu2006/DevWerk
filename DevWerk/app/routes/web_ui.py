  ...
  <div class="app-shell">
    <aside class="global-nav">
      ...
      <div class="nav-footer">
        <div class="identity"><span class="avatar">DW</span><span><b>DevWerk Console</b><small>Local backend UI</small></span></div>
...
.mini-usage { position: absolute; right: 14px; bottom: 12px; width: 112px; }
.mini-usage-label { color: var(--muted); font-size: 11px; font-weight: 800; margin-bottom: 4px; text-align: right; }
.mini-usage-bars { display: flex; justify-content: flex-end; gap: 3px; align-items: end; height: 26px; }
.mini-usage-bars i { width: 13px; background: linear-gradient(#8fb0ff, #eaf1ff); border-radius: 3px 3px 0 0; }
.mini-usage-legend { color: var(--muted); font-size: 10px; text-align: right; margin-top: 3px; }
...
.task-browser { display: grid; grid-template-columns: 280px minmax(0, 1fr) 320px; gap: 14px; align-items: start; }
.task-list-panel { padding: 14px; display: grid; gap: 10px; max-height: calc(100vh - 110px); overflow-y: auto; }
.task-list-item { width: 100%; text-align: left; border: 1px solid var(--border); background: #fff; border-radius: 9px; padding: 10px; cursor: pointer; }
.task-list-item.active { border-color: var(--blue); background: var(--blue-soft); }
.task-list-title { font-weight: 850; line-height: 1.35; }
.task-list-meta { color: var(--muted); font-size: 12px; margin-top: 6px; display: flex; justify-content: space-between; gap: 8px; }
...
@media (max-width: 1280px) {
  ...
  .config-grid, .kanban-page, .task-page, .task-browser { grid-template-columns: minmax(0,1fr); }
}
"""
...
function renderSettingsSection() {
  ...
  $("page").innerHTML = `<div class="section-grid">
    <section class="card card-pad"><div class="page-head"><div><h1 class="h2">Global Settings</h1><div class="muted">System-wide LLM APIs and route keys. Project workflow and agent settings live under Projects.</div></div><span class="badge blue">Global</span></div>
      <div class="dense-grid" style="margin-top:16px">${settingsTile("Default Route", (global.routing || {}).default || "default", "Default model route used when a project or agent has no override.")}${settingsTile("Route Keys", Object.keys(global.routing || {}).length || 0, "Route keys map agent responsibilities such as planning, coding, review, and compression to model configs.")}${settingsTile("LLM Providers", Object.keys(global.llms || {}).length || 0, "Configured provider blocks from llm.json or global settings API.")}</div>
      <div style="margin-top:14px" class="config-grid">${editorCard("Global LLM Catalog","Provider credentials, base URLs, and model definitions.","JSON", JSON.stringify(global.llms || {}, null, 2))}${editorCard("Global Routing Map","Agent responsibility keys mapped to model routes. These are not model names.","JSON", JSON.stringify(global.routing || {}, null, 2))}<div class="side-stack">${globalRoutingSummaryCard()}${settingsTile("About planning-agent", "Route key", "The old key 'planner' means the agent responsible for planning/design. It is a route alias, not a model name. Prefer clearer keys such as planning-agent in future configs.")}</div></div>
    </section>
...
function healthCard(){ const failed=allTasks().filter(t=>t.status_key==="failed").length; const active=activeStage(); return `<div class="card card-pad"><div style="display:flex;gap:10px;align-items:center"><span class="ok">${failed ? "!" : "✓"}</span><div><div class="h3">${failed ? "Attention" : "No failed tasks"}</div><div class="muted" style="font-size:12px">Derived from backend Kanban state.</div></div></div><div style="margin-top:18px" class="muted">Active Stage</div><div style="margin-top:8px"><span class="badge blue">${esc(active ? STAGE_TITLES[active] || active : "-")}</span></div><div class="muted" style="font-size:12px;margin-top:6px">${active ? statusCount(active) : 0} tasks in active stage</div><div class="progress"><span style="width:${active ? Math.max(3, Math.round(((STAGES.indexOf(active) + 1) / STAGES.length) * 100)) : 0}%"></span></div></div>`; }
...
function taskSummaryTab(task, events, artifacts){ return `<div class="summary-grid">
      ...
      <div class="summary-card wide"><div class="h3">Checklist</div><div class="check-list">${["Plan recorded","Context gathered","Implementation tracked","Review evidence available","Verification tracked","Memory updated"].map(x=>`<div class="check-item"><span class="ok">${taskHasEvidence(task, x) ? "✓" : "-"}</span><span>${x}</span></div>`).join("")}</div></div>
...
function taskPlanTab(task, artifacts){ const plans=artifacts.filter(a=>String(a.artifact_type || a.type || "").includes("plan") || JSON.stringify(a).includes("plan_bundle")); return `<div class="summary-grid"><div class="summary-card wide"><div class="h3">Plan Artifacts</div>${plans.length ? plans.map(a=>`<pre class="json-panel">${esc(JSON.stringify(a.payload || a, null, 2))}</pre>`).join("") : `<div class="muted">No plan artifacts returned by backend for task ${esc(task.id)}.</div>`}</div></div>`; }
...
function activeStage(){ if(state.activeTask && state.activeTask.status_key) return state.activeTask.status_key; const task = allTasks().find(t => ["coding","reviewed","ready_to_apply"].includes(t.status_key)); return task ? task.status_key : (columns()[0]?.status_key || ""); }
...
function routeKeyLabel(key){ const labels={planner:"Planning agent (planner)", planning:"Planning agent", "planning-agent":"Planning agent", coder:"Coding agent", coding:"Coding agent", executor:"Execution agent", reviewer:"Review agent", compression:"Compression agent", default:"Default route"}; return labels[key] || key; }
...
document.addEventListener("click", async event => {
  ...
  if (sectionLink && ["events","memory","analytics","settings"].includes(sectionLink.dataset.nav || "")) {
    event.preventDefault();
    state.page = "projects";
    state.section = sectionLink.dataset.nav;
    history.pushState(null, "", `/dashboard?project_id=${encodeURIComponent(state.projectId)}#${encodeURIComponent(state.section)}`);
    renderShell();
    return;
  }
