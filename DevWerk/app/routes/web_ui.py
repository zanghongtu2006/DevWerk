...
function renderSettingsSection() {
  ...
  $("page").innerHTML = `<div class="section-grid">
    <section class="card card-pad"><div class="page-head"><div><h1 class="h2">Global Settings</h1><div class="muted">System-wide LLM APIs and route keys. Project workflow and agent settings live under Projects.</div></div><span class="badge blue">Global</span></div>
      ...
      <div style="margin-top:14px" class="config-grid">${editorCard("Global LLM Catalog","Provider credentials, base URLs, and model definitions.","JSON", JSON.stringify(global.llms || {}, null, 2))}${editorCard("Global Routing Map","Model route aliases mapped to provider/model configs. These are not workflow columns or agent names.","JSON", JSON.stringify(global.routing || {}, null, 2))}<div class="side-stack">${globalRoutingSummaryCard()}${settingsTile("Dynamic Node Agents", "Workflow driven", "Project workflow nodes spawn temporary agents at runtime. Only project-agent and context-indexer are built in.")}</div></div>
    </section>
...
function taskSummaryTab(task, events, artifacts){ return `<div class="summary-grid">
      ...
      <div class="summary-card wide"><div class="h3">Checklist</div><div class="check-list">${["Plan recorded","Context gathered","Implementation tracked","Review evidence available","Verification tracked","Memory updated"].map(x=>`<div class="check-item"><span class="ok">${taskHasEvidence(task, x) ? "鉁? : "-"}</span><span>${x}</span></div>`).join("")}</div></div>
