...
from app.services.tool_protocol import ToolProtocolError, normalize_tool_requests
...
def _ask_project_conversation_agent(
    *,
    ...
    max_rounds = _positive_agent_rounds((metadata or {}).get("project_agent_max_rounds"), default=8, maximum=32)
    prompt = [
        {
            "role": "system",
            "content": (
                "You are DevWerk's project conversation agent. Return one JSON object only. "
                "You are a real agent with backend local tools, not a passive router. "
                ...
                "You may request backend tools when you need evidence or local OS work. Available tools: "
                "workspace.list, workspace.read, workspace.search, workspace.write, process.run, "
                "project.compile, source.diagnostics. If you need a tool, return tool_requests as an array "
                "and keep action=reply until the tool results come back. Use absolute paths when the user "
                "gives absolute paths. After tool_results are provided, continue reasoning and return a final "
                "action. Do not claim you cannot read or write local files when a backend tool can do it. "
                "JSON shape: {action, reply, save, task_id, task_request, notes, tool_requests}."
            ),
        },
        {
            "role": "user",
            "content": json.dumps(
                {
                    ...
                    "tool_protocol": {
                        "request_shape": {"id": "short-id", "tool": "workspace.read", "args": {"path": "absolute-or-project-relative-path"}},
                        "final_response": {"action": "reply|design|save_design|start_task|continue_task", "reply": "visible user message"},
                    },
    ...
    runtime_messages = list(prompt)
    try:
        for round_no in range(1, max_rounds + 1):
            decision = _project_conversation_chat_json_with_retry(project_id, runtime_messages)
            if not isinstance(decision, dict):
                raise HTTPException(status_code=502, detail="project LLM agent returned a non-object response")
            tool_requests = _project_agent_tool_requests(decision)
            add_project_event(
                project_id,
                "project_conversation_agent_round",
                {
                    "round": round_no,
                    "action": decision.get("action"),
                    "reply_preview": str(decision.get("reply") or "")[:1000],
                    "tool_requests": [request.model_dump() for request in tool_requests],
                },
            )
            if not tool_requests:
                return _normalize_project_agent_decision(decision)
            tool_results = _execute_project_conversation_tools(
                project_id=project_id,
            ...
            runtime_messages.append({"role": "assistant", "content": json.dumps(decision, ensure_ascii=False)})
                    ...
                    "content": json.dumps(
                        {
                            ...
                            "instruction": "Use these tool results to continue. Request another tool only if still necessary; otherwise return the final JSON action.",
                        },
                        ensure_ascii=False,
                    ),
                }
    ...
    raise HTTPException(status_code=504, detail=f"project conversation agent exceeded {max_rounds} tool rounds")


def _project_conversation_chat_json_with_retry(project_id: str, messages: list[dict[str, Any]]) -> dict[str, Any]:
    attempts = 3
    last_exc: BaseException | None = None
    for attempt in range(1, attempts + 1):
        try:
            return get_llm_client("project").chat_json(messages)
        except Exception as exc:  # noqa: BLE001
            last_exc = exc
            ...
            add_project_event(
                project_id,
                "project_conversation_llm_retry",
                {
                    "attempt": attempt,
...
def _project_agent_tool_requests(decision: dict[str, Any]) -> list[ToolRequest]:
    raw_requests = decision.get("tool_requests")
    if not raw_requests:
        return []
    if not isinstance(raw_requests, list):
        raise HTTPException(status_code=502, detail="project conversation agent returned non-array tool_requests")
    try:
        normalized = normalize_tool_requests(raw_requests)
    except ToolProtocolError as exc:
        raise HTTPException(status_code=502, detail=f"project conversation agent emitted invalid tool request: {exc}") from exc
    return [ToolRequest(**item) for item in normalized]


def _execute_project_conversation_tools(
    *,
    project_id: str,
    ...
    default_root = _project_agent_default_root(messages=messages, workspace=workspace, metadata=metadata)
    for request in requests:
        root = _project_agent_tool_root(request, default_root)
        add_project_event(
            project_id,
            "project_conversation_tool_requested",
            {
                "id": request.id,
                "tool": request.tool,
                "args": request.args,
        ...
        if root is None:
            result = {
                "id": request.id,
                "ok": False,
                "error": "project_root is required for relative backend local tool requests; use an absolute path or provide metadata.project_root",
            }
        else:
            result = execute_tool_requests([request], project_root=root)[0].model_dump()
        add_project_event(
            project_id,
            "project_conversation_tool_executed",
            {
                "id": result.get("id"),
                "tool": request.tool,
                "ok": result.get("ok"),
                "project_root": str(root) if root else None,
                "content_preview": str(result.get("content") or "")[:4000],
...
def _project_agent_default_root(
    *,
    messages: list[dict[str, Any]],
    workspace: dict[str, Any] | None,
    metadata: dict[str, Any],
    ...
    if isinstance(workspace, dict):
        ...
        candidates.extend(
            [
                ...
                workspace.get("workspace_root"),
                workspace.get("root"),
                source_map.get("root"),
            ]
        )
    candidates.extend(_extract_absolute_paths_from_messages(messages))
    for candidate in candidates:
        path = _as_existing_root(candidate)
        if path is not None:
            return path
    return None


def _project_agent_tool_root(request: ToolRequest, default_root: Path | None) -> Path | None:
    absolute = _first_absolute_path_from_tool(request)
    if absolute is not None:
        return _filesystem_anchor_root(absolute)
    return default_root


def _first_absolute_path_from_tool(request: ToolRequest) -> Path | None:
    args = request.args or {}
    candidates: list[object] = [args.get("path"), args.get("cwd")]
...
def _extract_absolute_paths_from_messages(messages: list[dict[str, Any]]) -> list[Path]:
    text = "\n".join(
...
def _as_existing_root(value: object) -> Path | None:
    ...
    try:
        ...
        return None
    if path.is_file():
        return _filesystem_anchor_root(path)
    if path.is_dir():
        return path
    parent = path.parent
...
def _filesystem_anchor_root(path: Path) -> Path:
    anchor = path.anchor or "/"
    return Path(anchor).resolve()


def _positive_agent_rounds(value: object, *, default: int, maximum: int) -> int:
    try:
        parsed = int(value)
